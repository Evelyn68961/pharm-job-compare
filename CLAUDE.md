...
- **Own-name build flag (`NEXT_PUBLIC_NEUTRAL`).** The site ships as **two Vercel projects off this one repo**: the default FJUH-boosted promo build (env unset → `FJUH_WIN_MULT = 4`, `FJUH_ALT_RATE = 0.5`), and the maintainer's own-name/Threads build (`NEXT_PUBLIC_NEUTRAL=1` → **gentle** `FJUH_WIN_MULT = 2`, `FJUH_ALT_RATE = 0.2`), served at `pharm-fortune.vercel.app`. The flag is read once in [fjuh.ts](app/lib/fjuh.ts). The gentle tier was chosen (over pure-neutral 1/0) so the FJUH **contact form** — the Threads-conversion test's KPI surface, gated by `isFjuh` and left ON — still appears often enough (~1-in-3 open / ~1-in-2 for 北北基 players, vs ~1-in-12 at pure neutral) while the 2× tilt stays invisible to readers. The flag keeps the name `NEUTRAL` for deploy continuity even though it now means "gentle", not zero-boost. Everything else is unchanged by the flag; if the own-name build ever needs the contact form hidden, gate [FjuhContactForm](app/components/spin/FjuhContactForm.tsx)/its call site on the same flag. Don't fork the repo — the whole point of the flag is one codebase, two deploys.
