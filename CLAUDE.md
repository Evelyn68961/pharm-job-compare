...
- **Neutral build flag (`NEXT_PUBLIC_NEUTRAL`).** The site ships as **two Vercel projects off this one repo**: the default FJUH-boosted promo build (env unset), and a neutral own-name/Threads build (`NEXT_PUBLIC_NEUTRAL=1`). The flag is read once in [fjuh.ts](app/lib/fjuh.ts): when set it forces `FJUH_WIN_MULT = 1` and `FJUH_ALT_RATE = 0`, so FJUH is ranked identically to every other hospital (no bias). Everything else — including the FJUH contact form (`isFjuh` gate) — is unchanged by the flag; the neutral build deliberately switches off **only** the weighting per owner decision. If neutral mode ever needs to also hide the contact form, gate [FjuhContactForm](app/components/spin/FjuhContactForm.tsx)/its call site on the same flag. Don't fork the repo for the neutral version — the whole point of the flag is one codebase, two deploys.
