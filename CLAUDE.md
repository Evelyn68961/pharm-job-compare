...
- [app/lib/resolveAlternatives.ts](app/lib/resolveAlternatives.ts) — top-3 recommendations + the FJUH visibility knobs (`FJUH_WIN_MULT`, `FJUH_ALT_RATE`, `isFjuh`)
...
- **The result is a 4-card swipe deck** ([ResultDeck.tsx](app/components/spin/ResultDeck.tsx)) — card 1 = winner (`✨ 你的命運醫院`), cards 2–4 = `resolveAlternatives` picks (`也推薦給你`). Horizontal CSS scroll-snap, no vertical scroll; dots + desktop arrows; footer = `再玩一次` + `輔大附醫藥劑部製作` credit. Each card has share + 104. Recommendation picks are up to 3 same-archetype hospitals (sorted by 薪資突出, falling back to other archetypes), with `idolRank[i]` driving each card's idol icon.
- **FJUH (輔大附醫) is deliberately boosted — this is intentional, not a bug.** Both knobs live in [resolveAlternatives.ts](app/lib/resolveAlternatives.ts) and are **region-gated** (they only fire when FJUH is already eligible, i.e. user picked 北北基 or no region — so FJUH never appears when its region was excluded). `FJUH_WIN_MULT` multiplies FJUH's wheel weight in `buildWheelCandidates` (wins the 命運醫院 slot more often, organically). `FJUH_ALT_RATE` is the probability of seeding FJUH into the recommendation cards at a random slot. Keep it subtle: don't make FJUH deterministic, don't bypass the region gate, and don't re-add a labelled "we made this / we're hiring" block (the old `關於這個網站` section was removed on purpose — the only standing FJUH mention is the one-line credit).
