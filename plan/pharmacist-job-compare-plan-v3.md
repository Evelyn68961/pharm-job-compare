...
>
> **In-season amendments** (post-approval, mid-build):
> §4 added `無大夜` as tag #11 under 生活/班別 (blue) — earned its place once
> 新國民 + 秉坤 showed up as anchor rows where 大夜 班 is structurally absent
> (distinct from 免/少輪班 which can mean either "免小夜" or "免大夜").
>
> **v3 changes from v2** (archived at
> [pharmacist-job-compare-plan-v2.md](pharmacist-job-compare-plan-v2.md)):
> §7 architecture wording aligned with §6 (Server Component reads Notion
> directly via `revalidate` — removed leftover "serverless function" framing
> from the Vite era); §5 mobile compare reframed from "ranking" to **grouping
> (not sorting)** — honest about text fields like 輪班說明 / 職務內容摘要 that
> can't be ranked; **sort UI dropped entirely** (§5 / §6 diagram + scope floor
> / §8 week 3 / §10) — no §3 field clears the bar for user-toggleable sort,
> default order is 薪資 tier (突出 first) with Notion insertion order as
> tiebreaker.
>
...
| 特色標籤 (tags) | [JUDGMENT] multi-select | the 11 curated tags below |
...
## 4. The curated feature tags (v1, fixed/positive-only)
...
| Group | Tags |
|---|---|
| 生活/班別 | 工作單純 · 免/少輪班 · 無大夜 · 夜班津貼優渥 |
| 規模/成長 | 醫學中心級 · 教學醫院 · 重視教學 · 全面藥事訓練 · 外派進修機會 |
| 加分福利 | 簽約金 · 提供宿舍 |

Each tag maps to evidence in the real postings (e.g. 工作單純 ← 三總「無化療無TPN·
工作單純」; 免/少輪班 ← 北榮桃園「免小夜·大夜每月0–2天」; 無大夜 ← 新國民「日班，
需輪值週六半天」/ 秉坤「上班時段只列日/晚/假日」; 重視教學 ← 輔大「醫學中心模式·PGY」).

`免/少輪班` and `無大夜` are distinct on purpose: 免/少輪班 covers any lightening
of the rotation (could be 免小夜 alone, like 北榮桃園 which still does 0–2 大夜/月);
無大夜 specifically signals **zero 大夜 班** — the most disruptive shift type
for pharmacists. A posting can earn one but not the other.
...
**Why ~10–12 is right for v1, not a frozen ceiling:**
...
- With ~15–20 jobs a season, ~10–12 tags means each tag lands on a meaningful
  chunk of listings, so filters actually narrow. More tags = each too sparse to
  be useful.
...
- **Tags are cheap to add later.** Launch with 10, watch which filters get used and
  what users ask for, add #11/#12 only when a real gap shows. (Done: `無大夜`
  added as #11 mid-season — see top-of-doc amendment.) Adding mid-season is a
  5-minute change; starting with 20 dead-weight tags is the expensive mistake.
...
3. **Compare** — tap a field, see *every* shortlisted job's value for just that
   field, side by side. One dimension at a time, all jobs visible, always
   readable on a phone.
   - **No ranking promised — this is grouping, not sorting.**
   - 薪資 tier naturally clusters into 突出 / 一般 (2 values; same grouping
     that drives the default list order).
   - All other fields — text (輪班說明, 職務內容摘要, 宿舍 …) and
     low-variance facts (更新日期, 學歷, 證照) — list each job's value for
     side-by-side reading without implying an order.
...
- **Default order:** 薪資 tier (突出 first), then Notion insertion order within
  tier. **No user-toggleable sort control.** Walked every §3 field against
  three bars — orderable, decision-driving when reordered, varies enough
  across rows — and nothing clears them: 薪資 tier is only 2 values (a
  grouping, not a sort); 更新日期 is uniformly fresh by Evelyn's weekly cadence;
  text fields aren't orderable; uniform fields (學歷, 證照) produce no
  reordering. A control with one option is decoration.
...
(data +        (holds secret key,                (cards, filters,
...
- One `"use client"` island for the filter/compare UI (local state).
...
3. The page's Server Component reads Notion directly (cached via
   `next.revalidate: 600` — see §6) and renders everything. Visitors get an
   instant, comparable view and click through to 104 for full detail.
...
| Week | Focus |
|---|---|
| 1 | Notion DB: define columns → **then** define the 11 tag options as multi-select → **then** seed the 5 known real jobs (order matters: tag options must exist before tagging rows, or you re-tag everything by hand). Next.js App Router project scaffold; Server Component fetching Notion with `revalidate: 600`. |
| 2 | Public site: card-list browse + desktop comparison table reading live data. |
| 3 | Mobile field-first compare; filters (輪班/地點/薪資門檻). Default order (突出 first, insertion order within tier) is the natural display order — no sort UI. |
...
- Tags #12+ if a real gap appears in-season (#11 已新增 `無大夜`，見頂部修訂).
...
- 11 fixed, positive, curated tags (§4); pick-from-list, not free-type.
- Desktop = side-by-side table; mobile = field-first compare; no shortlist cap.
- Filters 輪班/地點/薪資門檻. **No sort control** — default order = 薪資 tier
  (突出 first), Notion insertion order within tier. No accounts/favorites/search
  in v1.
